...
*鼠标在表格上移动显示效果
*/
jQuery(function(){bindMouseOver('table1', 'white', '#EFEFEF', '#DDE8EE');});
...
//	jQuery(this).bind("mouseover", function(){jQuery(this).children().css("backgroundColor", thisColor);});
	jQuery(this).bind("mouseover", function(){jQuery(this).children().toggleClass("tbody_tr_on");});
//	jQuery(this).bind("mouseout", function(){index % 2 ? jQuery(this).children().css("backgroundColor", firstColor) : jQuery(this).children().css("backgroundColor", secondColor);});
	jQuery(this).bind("mouseout", function(){index % 2 ? jQuery(this).children().toggleClass("tbody_tr2") : jQuery(this).children().toggleClass("tbody_tr_on");});
});
}
...
function showOssConfirmDialog(message,callbackFn){
	...
	oBuffer.push("<h1><img src=\"/itsm/images/knowledge/newindex/rr_r1_c22.gif\" /></h1>");
	oBuffer.push("</div>");
	...
	oBuffer.push("<img src=\"/itsm/images/knowledge/newindex/r2_c2.jpg\" />");
...
/********************
 * OssWorks确认对话框 
 * @param message 确认窗口中的提示信息带参数的
 * @param callbackFn 点击确定后的回调函数
 *******************/
function showOssConfirmDialogInParameters(message,callbackFn,par1,par2,par3){
	var oBuffer = new Array(); 
	oBuffer.push("<div id=\"windownbg\" class=\"windownbg\" style=\"height:"+jQuery(document).height()+"px;filter:alpha(opacity=0);opacity:1;z-index: 999901;\"></div>");
	oBuffer.push("<div id='ossDialog' style='position: absolute;_position: absolute;z-index: 999902'>");
	oBuffer.push("<div class=\"apdiv_top_yz\">");
...
function showSelectAndTextArea(isVisible){
	jQuery.each(jQuery("select"), function(i,obj){obj.style.display=(isVisible ? 'block' : 'none')});
	jQuery.each(jQuery("textArea"), function(i,obj){obj.style.display=(isVisible ? 'block' : 'none')});
}
/**
 * 用来切换div的显示和隐藏的
 * arrowId - 非必须参数。
 ...
 * msg 等待条的提示信息
 */
function showOssWaitMsg(msg){
	var oBuffer = new Array(); 
	oBuffer.push("<div id=\"windownbg\" class=\"windownbg\" style=\"height:"+jQuery(document).height()+"px;filter:alpha(opacity=0);opacity:1;z-index: 999901;\"></div>");
	oBuffer.push("<div id='ossWaitDialog' style='position: fixed;_position: absolute;z-index: 999902'>");
	oBuffer.push("<div class=\"jindu\">");
	oBuffer.push("<div class=\"wenzisuoming\">")
	oBuffer.push(msg);
	oBuffer.push("</div></div></div>");
	...
	var height = jQuery(document).scrollTop()+300;
	jQuery("#windownbg").show();
	jQuery("#windownbg").css("opacity","0");
	jQuery("#ossWaitDialog").css({
		    	"top" : height+"px",
		    	"left" : "500px"
		    }).show("fast");
...
function hideOssWaitMsg(){
	jQuery("#ossWaitDialog").remove();
	jQuery("#windownbg").remove();
}
...
function onlyForNumber(event) {
	...
	if((47<keyNum && keyNum<58) || keyNum == 8){
		return true;
	}else{
		return false;
	}
