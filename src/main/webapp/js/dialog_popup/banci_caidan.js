    ...
    $.popup = {
        ...
        _show: function(elem, msg, callback) {
            if ($("#_Popup_" + this.ID).length < 1) {
				...
				<div class="jinqi_cout">\
                  <div class="popup_header" id="_Title_"><h1>' + this.title + '</h1><h2 id="_ButtonRow_' + this.ID + '"></h2><div class="h_r"></div></div>\
                  <div class="popup_content">\
                    <div id="_Container_' + this.ID + (this.height == 0 ? '">' : '" style="height:' + this.height + 'px;line-height:26px">') + msg + '</div></div>' +
            ...
            switch (this.popType) {
                ...
                case 'prompt':
                    if (this.isButtonRow) {
                        ...
                        <input type="button" class="all_butt close" id="_ButtonOK_' + this.ID + '"/>');
                        $("#_ButtonOK_" + this.ID).click(function() {
						...
						$("samp").click(function() {
                           // var val = $("#hid_" + this.ID).val();
                            $.popup._hide();
                            //if (callback) callback(val);
							$("#li_id1").find("span").hide();
							$("#li_id1").find("select").show();
							$("#li_id1").find("input").show();
							$("#li_id1").find(".bianji_zy").removeClass("bianji_zy");
							//$("#sel").show();
                        });
