 ...
 * 
 * $Date: 2008-02-06 00:21:25 -0500 (Wed, 06 Feb 2008) $ $Rev: 4663 $
 */
...
jQuery.fn = jQuery.prototype = {
	init: function( selector, context ) {
		...
		return this.setArray(
			...
			// HANDLE: $(arraylike)
			// Watch for when an array-like object, contains DOM nodes, is
			// passed in as the selector
	...
	curCSS: function( elem, name, force ) {
		...
		if ( !force && elem.style && elem.style[ name ] )
			...
			// then some display: none elements are involved
				...
				// (It would be better if we knew the exact display type that
				// they had)
			...
			if ( t && !foundToken ) {
				...
				if ( !t.indexOf(",") ) {
					...
					if ( m[1] == "#" && elem && elem.getElementById && !jQuery.isXMLDoc(elem) ) {
						...
						// Do a quick check for the existence of the actual ID
						// attribute
						// to avoid selecting by the name attribute in IE
						// also check to insure id is a string to avoid
						// selecting an element with the name of 'id' inside a
						// form
	...
	trigger: function(type, data, elem, donative, extra) {
		...
		// Handle triggering a single element
			...
			if ( extra && jQuery.isFunction( extra ) ) {
				// call the extra function and tack the current return value on
	...
	if ( elem ) with ( jQuery.browser ) {
		...
		// Otherwise loop through the offsetParents and parentNodes
			...
			while ( offsetParent ) {
				...
				// Set offsetChild to previous offsetParent unless it is the
				// body element
