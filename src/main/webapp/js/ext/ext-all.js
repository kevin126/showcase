...
Ext.EventManager = function() {
	...
	var removeListener = function(el, ename, fn, scope) {
		...
		if (ename == "mousedown" && el == document && wrap) { // fix stopped
			// mousedowns on
			// the document
...
(function() {
	...
	El.prototype = {
		...
		autoHeight : function(animate, duration, onComplete, easing) {
			...
			setTimeout(function() {
						...
						if (!animate) {
							...
							this.setHeight(oldHeight); // restore original
							// height
		...
		setOverflow : function(v) {
			if (v == 'auto' && Ext.isMac && Ext.isGecko2) { // work around
				// stupid FF 2.0/Mac
				// scroll bar bug
...
Ext.Updater = Ext.extend(Ext.util.Observable, {
			...
			processSuccess : function(response) {
				...
				if (response.argument.form && response.argument.reset) {
					try { // put in try/catch since some older FF releases had
						// problems with this
...
(function() {
	...
	Ext.apply(Date, {
		...
		getMonthNumber : function(name) {
			...
			// the Date.monthNumbers hash are case sensitive)
		...
		formatCodes : {
			...
			D : "Date.getShortDayName(this.getDay())", // get localised short
			// day name
			j : "this.getDate()",
			...
			M : "Date.getShortMonthName(this.getMonth())", // get localised
			// short month name
			n : "(this.getMonth() + 1)",
			...
			c : function() { // ISO-8601 -- GMT format
				for (var c = "Y-m-dTH:i:sP", code = [], i = 0, l = c.length; i < l; ++i) {
					var e = c.charAt(i);
					code.push(e == "T" ? "'T'" : Date.getFormatCode(e)); // treat
					// T as
					// a
					// character
					// literal
		...
		getFormatCode : function(character) {
			...
			if (f) {
				...
				Date.formatCodes[character] = f; // reassign function result
				// to prevent repeated
				// execution
			}
		...
		createParser : function() {
			var code = [
					...
					"{2}",
					"if(u){",
					"v = new Date(u * 1000);", // give top priority to UNIX
					// time
					...
					"}",
					"}",
					"return (v && (z != null || o != null))?" // favour UTC
							// offset over
							// GMT offset
							+ " (Ext.type(z) == 'number' ? v.add(Date.SECOND, -v.getTimezoneOffset() * 60 - z) :" // reset
							// to
							// UTC,
							// then
							// add
		...
		parseCodes : {
			...
			j : {
				g : 1,
				c : "d = parseInt(results[{0}], 10);\n",
				s : "(\\d{1,2})" // day of month without leading zeroes (1 -
				// 31)
			},
			D : function() {
			...
			z : {
				g : 0,
				c : null,
				s : "(?:\\d{1,3})" // day of the year (0 - 364 (365 in leap
				// years))
			},
			W : {
				g : 0,
			...
			M : function() {
				for (var a = [], i = 0; i < 12; a.push(Date
						.getShortMonthName(i)), ++i); // get localised short
				// month names
				return Ext.applyIf({
							s : "(" + a.join("|") + ")"
						}, $f("F"));
			...
			n : {
				g : 1,
				c : "m = parseInt(results[{0}], 10) - 1;\n",
				s : "(\\d{1,2})" // month number without leading zeros (1 -
				// 12)
			},
			t : {
				g : 0,
			...
			H : {
				g : 1,
				c : "h = parseInt(results[{0}], 10);\n",
				s : "(\\d{2})" // 24-hr format of an hour with leading zeroes
				// (00 - 23)
			},
			i : {
				g : 1,
			...
			u : {
				...
				s : "(\\d+)" // decimal fraction of a second (minimum = 1
				// digit, maximum = unlimited)
			},
			O : {
				g : 1,
				c : [
			...
			T : {
				g : 0,
				c : null,
				s : "[A-Z]{1,4}" // timezone abbrev. may be between 1 - 4
				// chars
			},
			Z : {
				g : 1,
			...
			c : function() {
				var calc = [], arr = [$f("Y", 1), // year
						...
						// maximum = unlimited)
						{
							c : [ // allow both "Z" (i.e. UTC) and "+08:00"
									// (i.e. UTC offset) time zone delimiters
									"if(results[9] == 'Z'){", "z = 0;",
									"}else{", $f("P", 9).c, "}"].join('\n')
						}];
				...
				return {
					...
					s : arr[0].s + "-" + arr[1].s + "-" + arr[2].s + "T"
							...
							+ "((\.|,)\\d+)?" // decimal fraction of a second
							// (e.g. ",998465" or ".998465")
							+ "(Z|([+\-]\\d{2}:\\d{2}))" // "Z" (UTC) or
							// "+08:00" (UTC
					// offset)
				}
			...
			U : {
				...
				s : "(-?\\d+)" // leading minus sign indicates seconds before
				// UNIX epoch
			}
		}
	});
...
Ext.apply(Date.prototype, {
	...
	getTimezone : function() {
		// the following list shows the differences between date strings from
		// different browsers on a WinXP SP2 machine from an Asian locale:
		//
		// Opera : "Thu, 25 Oct 2007 22:53:45 GMT+0800" -- shortest (weirdest)
		// date string of the lot
		// Safari : "Thu Oct 25 2007 22:55:35 GMT+0800 (Malay Peninsula Standard
		// Time)" -- value in parentheses always gives the correct timezone
		// (same as FF)
		// FF : "Thu Oct 25 2007 22:55:35 GMT+0800 (Malay Peninsula Standard
		// Time)" -- value in parentheses always gives the correct timezone
		// IE : "Thu Oct 25 22:54:35 UTC+0800 2007" -- (Asian system setting)
		// look for 3-4 letter timezone abbrev
		// IE : "Thu Oct 25 17:06:37 PDT 2007" -- (American system setting) look
		// for 3-4 letter timezone abbrev
		//
		// this crazy regex attempts to guess the correct timezone abbreviation
		// despite these differences.
		// step 1: (?:\((.*)\) -- find timezone in parentheses
		// step 2: ([A-Z]{1,4})(?:[\-+][0-9]{4})?(?: -?\d+)?) -- if nothing was
		// found in step 1, find timezone from timezone offset portion of date
		// string
	...
	getWeekOfYear : function() {
		...
		return function() { // return a closure so constants get calculated only
			// once
			var DC3 = Date.UTC(this.getFullYear(), this.getMonth(), this
							.getDate()
							+ 3)
	...
	add : function(interval, value) {
		...
		switch (interval.toLowerCase()) {
			...
			case Date.MONTH :
				...
				if (day > 28) {
					day = Math.min(day, this.getFirstDateOfMonth().add('mo',
									value).getLastDateOfMonth().getDate());
				}
				d.setDate(day);
...
Ext.util.TaskRunner = function(interval) {
	...
	var runTasks = function() {
		...
		for (var i = 0, len = tasks.length; i < len; ++i) {
			...
			if (t.interval <= itime) {
				var rt = t.run
						.apply(t.scope || t, t.args || [++t.taskRunCount]);
				t.taskRunTime = now;
...
Ext.extend(Ext.util.MixedCollection, Ext.util.Observable, {
			...
			item : function(key) {
				var item = typeof this.map[key] != "undefined"
						? this.map[key]
						: this.items[key];
				return typeof item != 'function' || this.allowFunctions
						? item
						: null; // for prototype!
			},
			...
			keySort : function(dir, fn) {
				this._sort("key", dir, fn || function(a, b) {
							var v1 = String(a).toUpperCase(), v2 = String(b)
									.toUpperCase();
			...
			getRange : function(start, end) {
				...
				start = start || 0;
				end = Math.min(typeof end == "undefined"
								? this.length - 1
								: end, this.length - 1);
				var r = [];
				if (start <= end) {
			...
			createValueMatcher : function(value, anyMatch, caseSensitive) {
				if (!value.exec) { // not a regex
					value = String(value);
					value = new RegExp((anyMatch === true ? '' : '^')
									+ Ext.escapeRe(value), caseSensitive
									? ''
									: 'i');
				}
				return value;
...
Ext.XTemplate = function() {
	...
	while (m = s.match(re)) {
		...
		if (m3) {
			...
			if (exp) {
				fn = new Function('values', 'parent', 'xindex', 'xcount',
						'with(values){ return '
		...
		if (m4) {
			...
			if (exp) {
				exec = new Function('values', 'parent', 'xindex', 'xcount',
						'with(values){ ' + (Ext.util.Format.htmlDecode(exp))
								+ '; }');
			}
		}
		if (name) {
	...
	applySubTemplate : function(id, values, parent, xindex, xcount) {
		...
		if (t.target && Ext.isArray(vs)) {
			...
			for (var i = 0, len = vs.length; i < len; i++) {
				buf[buf.length] = t.compiled.call(this, vs[i], parent, i + 1,
						len);
			}
			return buf.join('');
		}
...
Ext.KeyMap.prototype = {
	...
	addBinding : function(config) {
		...
		var keyCode = config.key, shift = config.shift, ctrl = config.ctrl, alt = config.alt, fn = config.fn
				|| config.handler, scope = config.scope;
		...
		var handler = function(e) {
			if ((!shift || e.shiftKey) && (!ctrl || e.ctrlKey)
					&& (!alt || e.altKey)) {
				var k = e.getKey();
				if (keyArray) {
	...
	Ext.dd.DragDropMgr = function() {
		...
		return {
			...
			handleMouseDown : function(e, oDD) {
				...
				if (this.dragCurrent) {
					// the original browser mouseup wasn't handled (e.g. outside
					// FF browser window)
			...
			fireEvents : function(e, isDrop) {
				...
				// Check to see if the object(s) we were hovering over is no
				// longer
				...
				if (len == 1) {
					...
					for (var i = 0; i < len; ++i) {
						...
						if (dd.cursorIsOver) {
							...
							if (!winner
									|| winner.overlap.getArea() < dd.overlap
											.getArea()) {
								winner = dd;
							}
						}
					...
					for (var i in this.ids[sGroup]) {
						...
						if (this.isTypeOfDD(oDD)) {
							...
							if (loc) {
								...
								// this will unregister the drag and drop object
								// if
			...
			isOverTarget : function(pt, oTarget, intersect) {
				...
				// mousedown
				// in this case we are done. In POINT mode, if the drag obj has
				// no
				// contraints, we are also done. Otherwise we need to evaluate
				// the
				// location of the target as related to the actual location of
				// the
				// dragged element.
				var dc = this.dragCurrent;
				...
				var el = dc.getDragEl();
				var curRegion = new Ext.lib.Region(pos.y, pos.x
								+ el.offsetWidth, pos.y + el.offsetHeight,
						pos.x);
			...
			getElWrapper : function(id) {
				...
				if (!oWrapper || !oWrapper.el) {
					oWrapper = this.elementCache[id] = new this.ElementWrapper(Ext
							.getDom(id));
				}
				return oWrapper;
			},
			...
			handleWasClicked : function(node, id) {
				if (this.isHandle(id, node.id)) {
					...
					// check to see if this is a text node child of the one we
					// want
...
Ext.extend(Ext.dd.DDProxy, Ext.dd.DD, {
			...
			createFrame : function() {
				...
				if (!div) {
					...
					// appendChild can blow up IE if invoked prior to the window
					// load event
					// while rendering a table. It is possible there are other
					// scenarios
			...
			endDrag : function(e) {
				...
				this.beforeMove();
				// Hide the linked element before the move to get around a
				// Safari
				// rendering bug.
...
Ext.extend(Ext.dd.DragSource, Ext.dd.DDProxy, {
	...
	onDragDrop : function(e, id) {
		...
		if (this.beforeDragDrop(target, e, id) !== false) {
			if (target.isNotifyTarget) {
				if (target.notifyDrop(this, e, this.dragData)) { // valid
					// drop?
	...
	onInvalidDrop : function(target, e, id) {
		...
		this.proxy.repair(this.getRepairXY(e, this.dragData), this.afterRepair,
				this);
...
Ext.extend(Ext.dd.DropZone, Ext.dd.DropTarget, {
			...
			notifyDrop : function(dd, e, data) {
				...
				var n = this.getTargetFromEvent(e);
				return n ? this.onNodeDrop(n, dd, e, data) : this
						.onContainerDrop(dd, e, data);
			},
...
Ext.data.DataReader = function(meta, recordType) {
	...
	this.meta = meta;
	this.recordType = Ext.isArray(recordType) ? Ext.data.Record
...
Ext.extend(Ext.data.ScriptTagProxy, Ext.data.DataProxy, {
			...
			destroyTrans : function(trans, isLoaded) {
				...
				if (isLoaded) {
					...
					// if hasn't been loaded, wait for load to remove it to
...
Ext.extend(Ext.data.XmlReader, Ext.data.DataReader, {
			...
			readRecords : function(doc) {
				...
				if (this.meta.totalRecords) {
					totalRecords = q.selectNumber(this.meta.totalRecords, root,
							0);
				}
...
Ext.extend(Ext.Component, Ext.util.Observable, {
	...
	render : function(container, position) {
		if (!this.rendered && this.fireEvent("beforerender", this) !== false) {
			...
			// 验证权限
			if (!Ext.isEmpty(this.isPermission)) {
				this.setVisible(this.isPermission);
			}
			//把时间格式化为年-月-日
			
			// 验证是自己还是管理员
			if (!Ext.isEmpty(this.isAdmin)) {
				if (this.isAdmin) {
	...
	getAutoCreate : function() {
		var cfg = typeof this.autoCreate == "object" ? this.autoCreate : Ext
	...
	focus : function(selectText, delay) {
		if (delay) {
			this.focus.defer(typeof delay == 'number' ? delay : 10, this, [
							selectText, false]);
			return;
		}
		if (this.rendered) {
	...
	isXType : function(xtype, shallow) {
		...
		if (typeof xtype == 'function') {
			xtype = xtype.xtype; // handle being passed the class, eg.
			// Ext.Component
		} else if (typeof xtype == 'object') {
			xtype = xtype.constructor.xtype; // handle being passed an
			// instance
		}

		return !shallow ? ('/' + this.getXTypes() + '/').indexOf('/' + xtype
				+ '/') != -1 : this.constructor.xtype == xtype;
	},
	...
	// internal function for auto removal of assigned event handlers on
	// destruction
...
(function() {
	...
	Ext.extend(Ext.Layer, Ext.Element, {
		...
		// private
		// this code can execute repeatedly in milliseconds (i.e. during a drag)
		// so
		// code size was sacrificed for effeciency (e.g. no getBox/setBox, no XY
		// calls)
		...
		// private
		showAction : function() {
			this.visible = true; // track visibility to prevent getStyle
			// calls
...
Ext.BoxComponent = Ext.extend(Ext.Component, {
			...
			// private, set in afterRender to signify that the component has
			// been rendered
			boxReady : false,
			...
			setPagePosition : function(x, y) {
				...
				if (x === undefined || y === undefined) { // cannot translate
					// undefined points
					return;
				}
...
Ext.SplitBar = function(dragElement, resizingElement, orientation, placement,
		existingProxy) {
...
Ext.extend(Ext.SplitBar, Ext.util.Observable, {
			onStartProxyDrag : function(x, y) {
				...
				if (this.orientation == Ext.SplitBar.HORIZONTAL) {
					...
					this.dd.setXConstraint(this.placement == Ext.SplitBar.LEFT
									? c1
									: c2, this.placement == Ext.SplitBar.LEFT
									? c2
									: c1);
					...
					this.dd.setYConstraint(this.placement == Ext.SplitBar.TOP
									? c1
									: c2, this.placement == Ext.SplitBar.TOP
									? c2
									: c1);
				}
			...
			onEndProxyDrag : function(e) {
				...
				newSize = Math.min(Math.max(newSize, this.activeMinSize),
						this.activeMaxSize);
	...
	setElementSize : function(s, newSize, onComplete) {
		if (s.orientation == Ext.SplitBar.HORIZONTAL) {
			...
			if (!s.animate) {
				...
				s.resizingEl
						.setHeight(newSize, true, .1, onComplete, 'easeOut');
			}
		}
	}
};
...
Ext.SplitBar.AbsoluteLayoutAdapter.prototype = {
	...
	setElementSize : function(s, newSize, onComplete) {
		this.basic.setElementSize(s, newSize, this.moveSplitter.createDelegate(
						this, [s]));
	},
	...
	moveSplitter : function(s) {
		...
		switch (s.placement) {
			...
			case yes.RIGHT :
				s.el.setStyle("right",
						(this.container.getWidth() - s.resizingEl.getLeft())
								+ "px");
				break;
			case yes.TOP :
...
Ext.Container = Ext.extend(Ext.BoxComponent, {
			...
			remove : function(comp, autoDestroy) {
				...
				if (c && this.fireEvent('beforeremove', this, c) !== false) {
					...
					delete c.ownerCt;
					if (autoDestroy === true
							|| (autoDestroy !== false && this.autoDestroy)) {
						c.destroy();
					}
			...
			onLayout : function(ct, target) {
				...
				for (i = 0; i < len; i++) {
					...
					if (c.anchor) {
						...
						cw = a.right
								? this.adjustWidthAnchor(a.right(w), c)
								: undefined;
						ch = a.bottom
								? this.adjustHeightAnchor(a.bottom(h), c)
								: undefined;
...
Ext.layout.FormLayout = Ext.extend(Ext.layout.AnchorLayout, {
	...
	setContainer : function(ct) {
		...
		if (ct.hideLabels) {
			...
			if (typeof ct.labelWidth == 'number') {
				...
				this.elementStyle = "padding-left:" + (ct.labelWidth + pad)
						+ 'px';
			}
	...
	adjustWidthAnchor : function(value, comp) {
		return value
				- (comp.isFormField
						? (comp.hideLabel ? 0 : this.labelAdjust)
						: 0);
	},
...
Ext.layout.AbsoluteLayout = Ext.extend(Ext.layout.AnchorLayout, {
			...
			// private
			setContainer : function(ct) {
				Ext.layout.AbsoluteLayout.superclass.setContainer
						.call(this, ct);
				if (ct.isXType('form')) {
					this.isForm = true;
				}
...
Ext.Panel = Ext.extend(Ext.Container, {
	...
	onRender : function(ct, position) {
		...
		// This block allows for maximum flexibility and performance when using
		// existing markup
	...
	addTool : function() {
		...
		for (var i = 0, a = arguments, len = a.length; i < len; i++) {
			...
			if (!this.tools[tc.id]) {
				...
				var t = this.toolTemplate
						.insertFirst((tc.align !== 'left')
										? this[this.toolTarget]
										: this[this.toolTarget].child('span'),
								tc, true);
				this.tools[tc.id] = t;
...
Ext.Window = Ext.extend(Ext.Panel, {
	...
	beforeResize : function() {
		this.resizer.minHeight = Math.max(this.minHeight, this.getFrameHeight()
						+ 40); // 40 is a magic minimum content size?
	...
	afterShow : function() {
		...
		if (Ext.isMac && Ext.isGecko) { // work around stupid FF 2.0/Mac scroll
			// bar bug
	...
	unghost : function(show, matchPosition) {
		if (show !== false) {
			this.el.show();
			this.focus();
			if (Ext.isMac && Ext.isGecko) { // work around stupid FF 2.0/Mac
				// scroll bar bug
				this.cascade(this.setAutoScroll);
			}
		}
	...
	// private
	// z-index is managed by the WindowManager and may be overwritten at any
	// time
...
Ext.DataView = Ext.extend(Ext.BoxComponent, {
	...
	collectData : function(records, startIndex) {
		var r = [];
		for (var i = 0, len = records.length; i < len; i++) {
			r[r.length] = this.prepareData(records[i].data, startIndex + i,
					records[i]);
		}
		return r;
	},
	...
	doMultiSelection : function(item, index, e) {
		if (e.shiftKey && this.last !== false) {
			...
			if ((e.ctrlKey || this.simpleSelect) && this.isSelected(index)) {
				...
				this
						.select(index, e.ctrlKey || e.shiftKey
										|| this.simpleSelect);
			}
		}
	},
...
Ext.TabPanel = Ext.extend(Ext.Panel, {
	...
	onItemIconChanged : function(item, iconCls, oldCls) {
		var el = this.getTabEl(item);
		if (el) {
			Ext.fly(el).child('span.x-tab-strip-text').replaceClass(oldCls,
					iconCls);
		}
	},

	getTabEl : function(item) {
		var itemId = (typeof item === 'number') ? this.items.items[item]
		...
		if (this.el) {
			...
			if (Ext.isIE7 && Ext.isStrict) {
				...
				if (ib && ib.getWidth() > 20) {
					ib.clip();
					ib
							.setWidth(Ext.util.TextMetrics.measure(ib,
									this.text).width
									+ ib.getFrameWidth('lr'));
				}
			}
...
Ext.extend(Ext.Editor, Ext.Component, {
	...
	onSpecialKey : function(field, e) {
		...
		if (this.field.triggerBlur
				&& (key == e.ENTER || key == e.ESC || key == e.TAB)) {
	...
	onShow : function() {
		...
		this.field.show();
		if (Ext.isIE && !this.fixIEFocus) { // IE has problems with focusing the
			// first time
	...
	var updateButtons = function(b) {
		...
		for (var k in buttons) {
			if (typeof buttons[k] != "function") {
				if (b[k]) {
					buttons[k].show();
					buttons[k].setText(typeof b[k] == "string"
							? b[k]
	...
	return {
		...
		getDialog : function(titleText) {
			if (!dlg) {
				...
				// TODO: refactor this block into a buttons config to pass into
		...
		updateText : function(text) {
			if (!dlg.isVisible() && !opt.width) {
				dlg.setSize(this.maxWidth, 100); // resize first so content
				// is never clipped from
				// previous shows
			}
			msgEl.update(text || '&#160;');
			...
			if (Ext.isIE && iw > 0) {
				// 3 pixels get subtracted in the icon CSS for an IE margin
				// issue,
				// so we have to add it back here for the overall width to be
				// consistent
				iw += 3;
			}
			var w = Math.max(Math.min(opt.width || iw + mw + fw + bw,
							this.maxWidth), Math.max(opt.minWidth
									|| this.minWidth, bwidth || 0));
			...
			if (Ext.isIE && w == bwidth) {
				w += 4; // Add offset when the content width is smaller than the
				// buttons.
			}
		...
		show : function(options) {
			...
			if (!d.isVisible()) {
				// force it to the end of the z-index stack so it gets a cursor
				// in FF
...
Ext.Tip.DD = function(tip, config) {
	Ext.apply(this, config);
	this.tip = tip;
	Ext.Tip.DD.superclass.constructor.call(this, tip.el.id, 'WindowDD-'
					+ tip.id);
...
Ext.QuickTip = Ext.extend(Ext.ToolTip, {
			...
			showAt : function(xy) {
				...
				if (t) {
					...
					if (t.align) { // TODO: this doesn't seem to work
						// consistently
...
Ext.tree.TreePanel = Ext.extend(Ext.Panel, {
			...
			proxyNodeEvent : function(ename, a1, a2, a3, a4, a5, a6) {
				if (ename == 'collapse' || ename == 'expand'
						|| ename == 'beforecollapse' || ename == 'beforeexpand'
...
Ext.tree.TreeEventModel.prototype = {
	...
	getNode : function(e) {
		...
		if (t = e.getTarget('.x-tree-node-el', 10)) {
			var id = Ext.fly(t, '_treeEvents').getAttributeNS('ext',
					'tree-node-id');
			if (id) {
...
Ext.tree.TreeNode = function(attributes) {
	...
	this.draggable = attributes.draggable !== false
			&& attributes.allowDrag !== false;
	this.allowChildren = attributes.allowChildren !== false
			&& attributes.allowDrop !== false;
	...
	var uiClass = this.attributes.uiProvider || this.defaultUI
			|| Ext.tree.TreeNodeUI;
...
Ext.extend(Ext.tree.TreeNode, Ext.data.Node, {
	...
	expand : function(deep, anim, callback) {
		if (!this.expanded) {
			...
			if (!this.isHiddenRoot()
					&& (this.getOwnerTree().animate && anim !== false) || anim) {
	...
	ensureVisible : function(callback) {
		...
		tree.expandPath(this.parentNode ? this.parentNode.getPath() : this
						.getPath(), false, function() {
					var node = tree.getNodeById(this.id); // Somehow if we
					// don't do this, we
					// lose changes that
					// happened to node
					// in the meantime
	...
	disable : function() {
		this.disabled = true;
		this.unselect();
		if (this.rendered && this.ui.onDisableChange) { // event without
			// subscribing
...
Ext.extend(Ext.tree.AsyncTreeNode, Ext.tree.TreeNode, {
			expand : function(deep, anim, callback) {
				if (this.loading) { // if an async load is already running,
					// waiting til it's done
					var timer;
					var f = function() {
				...
				if (!this.loaded) {
					...
					if (loader) {
						loader.load(this, this.loadComplete.createDelegate(
										this, [deep, anim, callback]));
						return;
					}
				}
...
Ext.tree.TreeNodeUI.prototype = {
	...
	renderIndent : function() {
		if (this.rendered) {
			...
			if (this.indentMarkup != indent) { // don't rerender if not
				// required
...
Ext.tree.TreeSorter = function(tree, config) {
	...
	this.sortFn = function(n1, n2) {
		...
		var v1 = sortType ? sortType(n1) : (cs
				? n1.attributes[p]
				: n1.attributes[p].toUpperCase());
		var v2 = sortType ? sortType(n2) : (cs
				? n2.attributes[p]
...
if (Ext.dd.DropZone) {
	...
	Ext.tree.TreeDropZone = function(tree, config) {
		...
		Ext.tree.TreeDropZone.superclass.constructor.call(this, tree.innerCt,
				config);
	...
	Ext.extend(Ext.tree.TreeDropZone, Ext.dd.DropZone, {
		...
		expandNode : function(node) {
			if (node.hasChildNodes() && !node.isExpanded()) {
				node.expand(false, null, this.triggerCacheRefresh
								.createDelegate(this));
			}
		},
		...
		isValidDropPoint : function(n, pt, dd, e, data) {
			...
			if ((pt == "above" || pt == "below")
					&& (targetNode.parentNode && targetNode.parentNode.allowChildren === false)) {
				return false;
			}
			if (dropNode
					&& (targetNode == dropNode || dropNode.contains(targetNode))) {
				return false;
			}
			// reuse the object
		...
		getDropPoint : function(e, n, dd) {
			var tn = n.node;
			if (tn.isRoot) {
				return tn.allowChildren !== false ? "append" : false; // always
				// append
				// for
				// root
			}
		...
		onNodeOver : function(n, dd, e, data) {
			...
			if (!this.expandProcId && pt == "append" && node.hasChildNodes()
					&& !n.node.isExpanded()) {
		...
		onNodeDrop : function(n, dd, e, data) {
			...
			var dropNode = data.node
					|| (dd.getTreeNode ? dd.getTreeNode(data, targetNode,
							point, e) : null);
			var dropEvent = {
				tree : this.tree,
				target : targetNode,
		...
		removeDropIndicators : function(n) {
			if (n && n.ddel) {
				var el = n.ddel;
				Ext.fly(el).removeClass(["x-tree-drag-insert-above",
...
if (Ext.dd.DragZone) {
	Ext.tree.TreeDragZone = function(tree, config) {
		Ext.tree.TreeDragZone.superclass.constructor.call(this, tree
						.getTreeEl(), config);
...
Ext.extend(Ext.tree.TreeEditor, Ext.Editor, {
			...
			fitToTree : function(ed, el) {
				...
				if (td.scrollLeft > nd.offsetLeft) { // ensure the node left
					// point is visible
			...
			triggerEdit : function(node, defer) {
				...
				if (node.attributes.editable !== false) {
					...
					this.autoEditTimer = this.startEdit.defer(this.editDelay,
							this, [node.ui.textNode, node.text]);
					return false;
				}
			},
...
Ext.menu.MenuMgr = function() {
	...
	function onShow(m) {
		...
		if (m.parentMenu) {
			m.getEl().setZIndex(parseInt(m.parentMenu.getEl()
							.getStyle("z-index"), 10)
					+ 3);
			m.parentMenu.activeChild = m;
		} else if (last && last.isVisible()) {
			m.getEl().setZIndex(parseInt(last.getEl().getStyle("z-index"), 10)
					+ 3);
		}
	}
	...
	return {
		...
		get : function(menu) {
			if (typeof menu == "string") { // menu id
				...
				return menu;
			} else if (typeof menu.length == 'number') { // array of menu
				// items?
...
Ext.extend(Ext.menu.Item, Ext.menu.BaseItem, {
			...
			// private
			handleClick : function(e) {
				if (!this.href) { // if no link defined, stop the event
					// automatically
					e.stopEvent();
				}
			...
			shouldDeactivate : function(e) {
				if (Ext.menu.Item.superclass.shouldDeactivate.call(this, e)) {
					if (this.menu && this.menu.isVisible()) {
						return !this.menu.getEl().getRegion().contains(e
								.getPoint());
					}
					return true;
				}
...
Ext.extend(Ext.menu.CheckItem, Ext.menu.Item, {
	...
	setChecked : function(state, suppressEvent) {
		if (this.checked != state
				&& this.fireEvent("beforecheckchange", this, state) !== false) {
			if (this.container) {
	...
	// private
	handleClick : function(e) {
		if (!this.disabled && !(this.checked && this.group)) {// disable
			// unselect on
			// radio item
...
Ext.menu.DateItem = function(config) {
	Ext.menu.DateItem.superclass.constructor.call(this,
			new Ext.DatePicker(config), config);
...
Ext.menu.ColorItem = function(config) {
	Ext.menu.ColorItem.superclass.constructor.call(this,
			new Ext.ColorPalette(config), config);
...
Ext.form.Field = Ext.extend(Ext.BoxComponent, {
			...
			initEvents : function() {
				...
				var o = this.inEditor && Ext.isWindows && Ext.isGecko ? {
					buffer : 10
				} : null;
			...
			// protected - should be overridden by subclasses if necessary to
			// prepare raw values for validation
			processValue : function(value) {
				return value;
			},

			// private
			// Subclasses should provide the validation implementation by
			// overriding this
			...
			// private
			getErrorCt : function() {
				return this.el.findParent('.x-form-element', 5, true) || // use
						// form
						// element
						// wrap
						// if
						// available
						this.el.findParent('.x-form-field-wrap', 5, true); // else
				// direct
				// field
				// wrap
			},
			...
			getRawValue : function() {
				var v = this.rendered ? this.el.getValue() : Ext.value(
						this.value, '');
				if (v === this.emptyText) {
					v = '';
				}
			...
			setRawValue : function(v) {
				return this.el.dom.value = (v === null || v === undefined
						? ''
						: v);
			},

			setValue : function(v) {
				this.value = Ext.util.Format.htmlDecode(v);
				if (this.rendered) {
...
Ext.form.TextField = Ext.extend(Ext.form.Field, {
	...
	autoSize : function() {
		...
		d = null;
		v += "&#160;";
		var w = Math.min(this.growMax, Math.max(this.metrics.getWidth(v) + 10,
						this.growMin));
		this.el.setWidth(w);
...
Ext.form.TriggerField = Ext.extend(Ext.form.TextField, {
			...
			// private
			// This should be overriden by any subclass that needs to check
			// whether or not the field can be blurred.
...
// abstract base class
// to be extended by an implementing class. For an example of implementing this
// class, see the custom
...
Ext.form.Radio = Ext.extend(Ext.form.Checkbox, {
			...
			toggleValue : function() {
				if (!this.checked) {
					var els = this.getParent().select('input[name='
							+ this.el.dom.name + ']');
	...
	render : function() {
		this.items.each(function(f) {
					if (f.isFormField && !f.rendered
							&& document.getElementById(f.id)) { // if the
						// element
						// exists
...
Ext.form.HtmlEditor = Ext.extend(Ext.form.Field, {
	...
	createFontOptions : function() {
		...
		for (var i = 0, len = fs.length; i < len; i++) {
			...
			buf.push('<option value="', lc, '" style="font-family:', ff, ';"',
					(this.defaultFont == lc ? ' selected="true">' : '>'), ff,
					'</option>');
		}
		return buf.join('');
	},
	...
	createToolbar : function(editor) {
		...
		if (this.enableFontSize) {
			tb.add('-', btn('increasefontsize', false, this.adjustFont), btn(
...
Ext.form.TimeField = Ext.extend(Ext.form.ComboBox, {
	...
	// private - This is the date to use when generating time values in the
	// absence of either minValue
	// or maxValue. Using the current date causes DST issues on DST boundary
	// dates, so this is an
	...
	initComponent : function() {
		...
		if (!this.store) {
			...
			if (!max) {
				max = new Date(this.initDate).clearTime().add('mi',
						(24 * 60) - 1);
			}
			var times = [];
			while (min <= max) {
...
Ext.form.Label = Ext.extend(Ext.BoxComponent, {
			...
			setText : function(t, encode) {
				this.text = t;
				if (this.rendered) {
					this.el.dom.innerHTML = encode !== false ? Ext.util.Format
							.htmlEncode(t) : t;
				}
				return this;
			}
...
Ext.extend(Ext.grid.GridView, Ext.util.Observable, {
	...
	findCell : function(el) {
		...
		return this.fly(el).findParent(this.cellSelector,
	...
	syncHeaderScroll : function() {
		...
		this.innerHd.scrollLeft = mb.scrollLeft; // second time for IE (1/2
		// time first fails, other
		// browsers ignore)
	},
	...
	getColumnWidth : function(col) {
		...
		if (typeof w == 'number') {
			return (Ext.isBorderBox ? w : (w - this.borderWidth > 0 ? w
					- this.borderWidth : 0))
					+ 'px';
		}
		return w;
	...
	fitColumns : function(preventRefresh, onlyExpand, omitColumn) {
		...
		while (cols.length) {
			w = cols.pop();
			i = cols.pop();
			cm.setColumnWidth(i, Math.max(this.grid.minColumnWidth, Math
									.floor(w + w * frac)), true);
		}

		if ((tw = cm.getTotalWidth(false)) > aw) {
			var adjustCol = ac != vc ? omitColumn : extraCol;
			cm.setColumnWidth(adjustCol, Math.max(1, cm
									.getColumnWidth(adjustCol)
									- (tw - aw)), true);
		}
	...
	updateHeaderSortState : function() {
		...
		if (!this.sortState
				|| (this.sortState.field != state.field || this.sortState.direction != state.direction)) {
	...
	handleHdMove : function(e, t) {
		if (this.activeHd && !this.headersDisabled) {
			...
			if (x - r.left <= hw && this.cm.isResizable(this.activeHdIndex - 1)) {
				ss.cursor = Ext.isAir ? 'move' : Ext.isSafari
						? 'e-resize'
						: 'col-resize'; // col-resize not always supported
...
Ext.extend(Ext.grid.GridView.SplitDragZone, Ext.dd.DDProxy, {
			...
			endDrag : function(e) {
				...
				v.onColumnSplitterMoved(this.cellIndex, this.cm
...
Ext.grid.GroupingView = Ext.extend(Ext.grid.GridView, {
	...
	getCkBox : function() {
		if (this.ckBox) {
			return "<input type=checkbox value={[values.rs[0].get(\"status\")]} devId={[values.rs[0].get(\"deviceId\")]}  name=vlancks onclick=\"var ck = false;var number = 0;var ckValue = document.getElementsByName('vlancks');for(var i = 0; i < ckValue.length; i++){if (ckValue[i].checked) {number++;if(ckValue[i].value =='活动'){ck = true;}}};if(number == 0){ck = true}Ext.getCmp('delVlanId').setDisabled(ck);\">";
		} else {
			return "";
		}
	},
		...
		if (!this.startGroup) {
			this.startGroup = new Ext.XTemplate(
					...
					'<div id="{groupId}-hd" class="x-grid-group-hd" style="{style}"><div>'
							+ this.getCkBox(), this.groupTextTpl,
					'</div></div>',
	...
	onGroupByClick : function() {
		...
		this.beforeMenuShow(); // Make sure the checkboxes get properly set
	...
	// private
	onColumnWidthUpdated : function(col, w, tw) {
		Ext.grid.GroupingView.superclass.onColumnWidthUpdated.call(this, col,
				w, tw);
	...
	// private
	onAllColumnWidthsUpdated : function(ws, tw) {
		Ext.grid.GroupingView.superclass.onAllColumnWidthsUpdated.call(this,
				ws, tw);
		this.updateGroupWidths();
	},
	...
	// private
	onColumnHiddenUpdated : function(col, hidden, tw) {
		Ext.grid.GroupingView.superclass.onColumnHiddenUpdated.call(this, col,
				hidden, tw);
		this.updateGroupWidths();
	},
...
Ext.extend(Ext.grid.HeaderDragZone, Ext.dd.DragZone, {
			...
			onInitDrag : function(e) {
				...
				clone.id = Ext.id();
				clone.style.width = Math.min(this.dragData.header.offsetWidth,
						this.maxDragWidth)
						+ "px";
				this.proxy.update(clone);
				return true;
...
Ext.extend(Ext.grid.HeaderDropZone, Ext.dd.DropZone, {
			...
			positionIndicator : function(h, n, e) {
				...
				if (oldIndex == newIndex
						&& (locked == this.grid.colModel.isLocked(oldIndex))) {
					return false;
				}
				...
				this.proxyBottom.setLeftTop(px, py
			...
			onNodeDrop : function(n, dd, e, data) {
				...
				if (h != n) {
					...
					var x = Ext.lib.Event.getPageX(e);
					var r = Ext.lib.Dom.getRegion(n.firstChild);
					var pt = (r.right - x) <= ((r.right - r.left) / 2)
							? "after"
							: "before";
					var oldIndex = this.view.getCellIndex(h);
					var newIndex = this.view.getCellIndex(n);
...
Ext.grid.GridView.ColumnDragZone = function(grid, hd) {
	Ext.grid.GridView.ColumnDragZone.superclass.constructor.call(this, grid,
			hd, null);
...
Ext.extend(Ext.grid.ColumnModel, Ext.util.Observable, {
	...
	isResizable : function(colIndex) {
		return colIndex >= 0 && this.config[colIndex].resizable !== false
...
Ext.extend(Ext.grid.RowSelectionModel, Ext.grid.AbstractSelectionModel, {
	...
	selectRow : function(index, keepExisting, preventViewNotify) {
		...
		if (r
				&& this.fireEvent("beforerowselect", this, index, keepExisting,
						r) !== false) {
...
Ext.ProgressBar = Ext.extend(Ext.BoxComponent, {
			...
			onRender : function(ct, position) {
				...
				var tpl = new Ext.Template('<div class="{cls}-wrap">',
			...
			updateProgress : function(value, text, animate) {
				...
				if (this.rendered) {
					...
					this.progressBar.setWidth(w, animate === true
									|| (animate !== false && this.animate));
					if (this.textTopEl) {
						// textTopEl should be the same width as the bar so
						// overflow will clip as the bar moves
...
Ext.Slider = Ext.extend(Ext.BoxComponent, {
	...
	onClickChange : function(local) {
		if (local.top > this.clickRange[0] && local.top < this.clickRange[1]) {
			this.setValue(Math.round(this.reverseValue(local.left)), undefined,
					true);
		}
	},
	...
	setValue : function(v, animate, changeComplete) {
		...
		if (v !== this.value
				&& this.fireEvent('beforechange', this, v, this.value) !== false) {
			this.value = v;
...
Ext.Slider.Vertical = {
	...
	onDrag : function(e) {
		...
		this.setValue(this.minValue + Math.round(bottom / this.getRatio()),
				false);
		this.fireEvent('drag', this, e);
	},
...
Ext.StatusBar = Ext.extend(Ext.Toolbar, {
	...
	clearStatus : function(o) {
		...
		var text = o.useDefaults ? this.defaultText : '', iconCls = o.useDefaults
	...
	Ext.apply(Ext, {
				...
				dump : function(o) {
					if (typeof o == 'string' || typeof o == 'number'
							|| typeof o == 'undefined' || Ext.isDate(o)) {
						Ext.log(o);
					} else if (!o) {
						Ext.log("null");
...
Ext.debug.DomTree = Ext.extend(Ext.tree.TreePanel, {
	...
	initComponent : function() {
		...
		function onNodeSelect(t, n, last) {
			...
			if (n && n.htmlNode) {
				...
				if (styles && !showAll.pressed) {
					...
					for (var a in dom) {
						var v = dom[a];
						if ((isNaN(a + 10)) && v != undefined && v !== null
								&& v !== ''
								&& !(Ext.isGecko && a[0] == a[0].toUpperCase())) {
							props[a] = v;
						}
					}
				}
			...
			for (var i = 0, c; c = cn[i]; i++) {
				...
				if (c.nodeType == 1) {
					...
					n.appendChild(new Ext.tree.TreeNode({
								text : '<em>'
										+ ellipsis(html(String(c.nodeValue)),
												35) + '</em>',
								cls : 'x-tree-noicon'
							}));
				}
		...
		// tree.getSelectionModel().on('selectionchange', onNodeSelect, null,
		// {buffer:250});
...
Ext.debug.HtmlNode = function() {
	...
	function renderNode(n, leaf) {
		...
		if (leaf && n.childNodes.length > 0) {
			s += '&gt;<em>' + ellipsis(html(String(n.innerHTML)), 35)
					+ '</em>&lt;/' + tag + '&gt;';
		} else if (leaf) {
			s += ' /&gt;';
		} else {
