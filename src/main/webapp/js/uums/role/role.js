...
editRole=function(roleId){
	...
	$("#disableDeleeBtn").show();
		 $.ajax({
				...
				dataType:'json',
				type:'post',
				url:'/uums/role/role!loadRoleJson.action',
...
doDeleteMember=function(){
	...
	var isNew=$("#isnewRole").val();
			if(isNew!=null){
				window.location.href="/uums/role/role!deleteRoleMember.action?new=true&&menuId=m12_1_1&memberId="+menberIdArray+"&roleId="+roleId;
			}else{
				window.location.href="/uums/role/role!deleteRoleMember.action?menuId=m12_1_1&memberId="+menberIdArray+"&roleId="+roleId;
			}
}
//向角色里面添加用户
addUserToRole=function(roleId){
		...
		url="/uums/common/query-user.action?selectOne=2&status=submit&enter=enter";
		}
...
function doCallBack(rtValue){
		...
		for(var i=0;i<rtValue.length;i++){
			...
			var roleId=document.getElementById("roleId").value;
			window.location.href="/uums/role/role!addUerToRole.action?new=true&menuId=m12_1_1&memberId="+returnId+"&roleId="+roleId;
}
function selectAllSubPermission(cunrretObj,actionId){
	var selectCheckbox=$("#"+actionId+" tr td input[type=checkbox]");
...
deleteRoleMember=function(){
	...
	window.location.href="/uums/role/role!deleteRoleMember.action?memberId="+userId+"&roleId="+roleId;
}
//删除角色取消按钮
cancelDeleteRoleAction=function(){
	hideDiv('delete');
}
